(function () {
  ...
  // --- Per-solution table HTMLs with updated content ---
  ...
  const tableHTMLs = [tableHTML0, tableHTML1, tableHTML2, tableHTML3, tableHTML4];
  
  const serviceQuestions = {
      '1.1': ["How many doctor logins do you need?", "How many pre-loaded reels are required?", "How many personalized frames do you prefer?", "What subscription duration works best for you?"],
      '1.2': ["What is the minimum number of doctors?", "Preferred campaign duration?", "Preferred Video Reel duration?"],
      '1.3': ["Per year frequency of Video Reels?", "How many doctors to engage?", "How many rep logins needed?"],
      '1.4': ["Who is the target audience for the platform?", "How often will webinars be conducted?", "Do you need custom branding (logo, colors, certificate)?", "How should pre-read materials be organized?"],
      '1.5': ["Which sections do you want in the hub?", "Need custom design/branding?", "Expected number of users at once?", "What expert content do you need?"],
      '1.6': ["What type of gamification do you prefer (quizzes, challenges, etc.)?", "Any specific campaign concepts you want?", "How many reps and doctors will use the system?", "What key metrics do you want in the dashboard?"],
      '2.1': ["What type of podcast content do you want (interviews, discussions, case studies)?", "Do you need custom host branding?", "How many episodes do you plan to produce?", "Do you want live streaming or recorded only?"],
      '2.2': ["What topics should the expert forums cover?", "Do you need branded community design?", "How many active members do you expect?", "Live discussions or only on-demand content?"],
      '2.3': ["What products will be launched?", "Do you want fully live or pre-recorded demos?", "Need custom 3D product models?", "Expected audience size for the event?"],
      '3.1': ["How many locations will participate in the hybrid meeting?", "What type of AV setup do you require (live streaming, recording, presentations)?", "Do you need pre-event and post-event reports?", "How many participants are expected overall?"],
      '3.2': ["How many locations need coverage?", "What AV setup is required (live streaming, recording, presentations)?", "Do you need pre-event and post-event reports?", "How many attendees per location?"],
      '3.3': ["Do you prefer live webinars or social media live sessions?", "How many sessions are planned in the series?", "Who will be the speakers (specialists, doctors)?", "Do you need interactive features (Q&A, polls)?"],
      '4.1': ["What key content do you want for HCPs (articles, trials, blogs)?", "What patient tracking features are most important (dose, symptoms, diary)?", "Do you need personalized content per HCP?", "How often should content be updated (real-time, monthly)?"],
      '4.2': ["Which specialties should the engine cover?", "What type of content should be recommended (news, clinical cases, patient resources)?", "Do you want recommendations to update in real-time or periodically?", "Should doctors be able to customize their interests manually?"],
      '4.3': ["What key features do you want for reps (Rx Tracker, sampling, Promotogram)?", "Do you need a custom analytics dashboard per rep?", "What type of gratitude system do you want (points, rewards, certificates)?", "How many reps and HCPs will use the system?"],
      '4.4': ["What types of surveys and PMS data do you want to track?", "Do you need filters by rep, doctor, city, or study?", "Should data be ready-to-publish on compliance standards?", "How frequently will surveys be conducted?"],
      '4.5': ["What content should each microsite include (achievements, clinic info, disease info)?", "Do you need regional language support?", "Any specific design or branding requirements for the microsites?", "How many doctors’ microsites need to be launched initially?"],
      '5.1': ["What topics should the livestream campaigns cover?", "Do you want anonymous Q&A for patients?", "Which social media platforms should be used?", "How many sessions are planned and their frequency?"],
      '5.2': ["How long is the campaign duration?", "How many doctors will participate?", "Do you want real-time booking or pre-scheduled slots?", "Any limits on the number of slots per doctor?"],
      '5.3': ["How many topics can each doctor choose?", "Are there any preferred topic categories?", "Will sessions be live only or recorded too?", "How will topics be reviewed or approved?"]
  };


  document.addEventListener('DOMContentLoaded', function () {
    const solutionBoxes = Array.from(document.querySelectorAll('.solution-box'));
    ...
    if (questionsCancelBtn) questionsCancelBtn.addEventListener('click', closeQuestionsModal);
    if (questionsSubmitBtn) questionsSubmitBtn.addEventListener('click', submitQuestions);
    ...
    function sendRemark() {
      ...
      remarkWrap.style.cssText = 'padding: 10px; border: 1px solid #ddd; margin-top: 15px; border-radius: 5px; background: #f9f9f9;';
      const time = new Date().toLocaleString('en-IN');
      remarkWrap.innerHTML = `<strong>Remark (${time}):</strong><div style="margin-top:6px; white-space:pre-wrap;">${escapeHtml(txt)}</div>`;
      modalDesc.appendChild(remarkWrap);
      closeRemarksModal();
    }

    if (addRemarksBtn) addRemarksBtn.addEventListener('click', openRemarksModal);
    if (remarksCancel) remarksCancel.addEventListener('click', closeRemarksModal);
